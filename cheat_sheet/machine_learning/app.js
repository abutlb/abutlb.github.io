...
const CAT_LABELS = {
  ...
  concepts:     '🧩 المفاهيم',
  supervised:   '🎯 إشراف',
  unsupervised: '🔍 بدون إشراف',
  workflow:     '⚙️ سير العمل',
  deep:         '🧠 تعلم عميق'
};
...
function showSection(name) {
  ...
  /* تشغيل progress bars عند فتح دراسة الحالة */
  if (name === 'casestudy') {
    setTimeout(() => {
        ...
        <div style="border-top:1px solid var(--border-subtle);
          ...
          <div>
            ...
            <div style="display:flex;flex-wrap:wrap;gap:.4rem">
              ${s.tools.map(tool => `
                <span class="badge">${tool}</span>`).join('')}
            </div>
          </div>
...
function showQuizResult() {
  ...
  if (pct >= 80) {
    emoji = '🏆'; title = 'ممتاز! أنت محلل بيانات محترف!';
    color = '#4ade80';
  } else if (pct >= 60) {
